...
for i in range(pn):
  plt.annotate(str(pc_alive_arr[i]), (-0.3+i, 40))
  if pc_alive_arr[i] < 40:
    plt.annotate(str(pc_dead_arr[i]), (-0.3+i, pc_alive_arr[i]+80))
  else:
    plt.annotate(str(pc_dead_arr[i]), (-0.3+i, pc_alive_arr[i]+30))
plt.show()
...
# 강사님 풀이
df.Embarked.isnull().value_counts() # 결측데이터 확인
edata = df.Embarked.fillna('N')
edata.isnull().value_counts()

evc = df.Embarked.value_counts()
print(type(evc)) # Series의 좌측 index, 우측 value
...
for index in range(len(edata)):
  ei = get_embarked_index(edata[index])
  if sur_data[index]==0:
    ec_dead_arr[ei] += 1
  else:
    ec_alive_arr[ei] += 1
...
for i in range(en):
  plt.annotate(str(ec_alive_arr[i]), (-0.3+i, 40))
  if ec_alive_arr[i] < 40:
    plt.annotate(str(ec_dead_arr[i]), (-0.3+i, ec_alive_arr[i]+80))
  else:
    plt.annotate(str(ec_dead_arr[i]), (-0.3+i, ec_alive_arr[i]+30))
plt.show()

ec_alive_arr, ec_dead_arr